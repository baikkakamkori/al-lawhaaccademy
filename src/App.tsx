...
import { 
  ...
  ArrowDownRight, ArrowUpRight, Image as ImageIcon, FileText 
} from 'lucide-react';
...
// Image processing to compress files for Firebase
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 overflow-x-hidden">
      {currentView === 'landing' && (
        <LandingPage 
          firebaseUser={firebaseUser} db={db} appId={appId} 
          onNavigateToLogin={(type) => { setDashboardType(type); setCurrentView('login'); }} 
        />
      )}
      {currentView === 'login' && (
        <LoginPage 
          type={dashboardType} onLogin={handleLogin} onBack={() => setCurrentView('landing')} 
          onSwitchType={(type) => setDashboardType(type)} 
        />
      )}
...
// --- LANDING PAGE ---
const LandingPage = ({ firebaseUser, db, appId, onNavigateToLogin }) => {
  ...
  useEffect(() => {
    ...
    try {
      onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'main_config'), (docSnap) => {
        if (docSnap.exists()) setSettings({ ...defaultSiteSettings, ...docSnap.data() });
      });
  ...
  const LogoDisplay = ({ className = "h-8 w-8" }) => (
    settings.logoUrl ? 
      <img src={settings.logoUrl} alt="Logo" className={`${className} object-contain`} /> : 
      <BookOpen className={`${className} text-emerald-800`} />
  );
    ...
    <div className="flex flex-col min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="md:hidden flex items-center">
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 rounded-md hover:bg-emerald-700 transition-colors">
      ...
      <div className="relative bg-emerald-900 text-white flex flex-col justify-center min-h-[70vh] md:min-h-[80vh]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 relative z-10 flex flex-col items-center text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto px-4 sm:px-0">
            ...
            <a href="#notices" className="w-full sm:w-auto border-2 border-emerald-500 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-emerald-800 transition flex justify-center items-center">নোটিশ দেখুন</a>
          </div>
        </div>
      </div>
      ...
      <div className="py-16 md:py-24 bg-white" id="about">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-emerald-50 rounded-3xl p-8 border border-emerald-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"><div className="bg-emerald-100 w-16 h-16 rounded-2xl flex items-center justify-center mb-6"><BookOpen className="h-8 w-8 text-emerald-700"/></div><h3 className="text-2xl font-bold text-gray-900 mb-3">কুরআনিক স্টাডিজ</h3><p className="text-gray-600 leading-relaxed">প্রত্যয়িত আলেমদের দ্বারা পরিচালিত হিফজ এবং তাজবীদ প্রোগ্রাম যা শিক্ষার্থীদের কুরআনের আলোয় আলোকিত করে।</p></div>
            <div className="bg-emerald-50 rounded-3xl p-8 border border-emerald-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"><div className="bg-emerald-100 w-16 h-16 rounded-2xl flex items-center justify-center mb-6"><Award className="h-8 w-8 text-emerald-700"/></div><h3 className="text-2xl font-bold text-gray-900 mb-3">আধুনিক কারিকুলাম</h3><p className="text-gray-600 leading-relaxed">বিশ্বমানের চ্যালেঞ্জ মোকাবেলায় জাতীয় পাঠ্যক্রমের সাথে বিজ্ঞান, প্রযুক্তি ও সাধারণ শিক্ষার চমৎকার সমন্বয়।</p></div>
            <div className="bg-emerald-50 rounded-3xl p-8 border border-emerald-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1"><div className="bg-emerald-100 w-16 h-16 rounded-2xl flex items-center justify-center mb-6"><Users className="h-8 w-8 text-emerald-700"/></div><h3 className="text-2xl font-bold text-gray-900 mb-3">চরিত্র গঠন (তারবিয়াহ)</h3><p className="text-gray-600 leading-relaxed">দৃঢ় নৈতিক চরিত্র, শৃঙ্খলা, সময়ানুবর্তিতা এবং সমাজসেবা বিকাশের উপর বিশেষ জোর দেওয়া হয়।</p></div>
          </div>
        </div>
      </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 flex-1 min-h-[400px] max-h-[500px] overflow-hidden flex flex-col">
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
                {notices.length > 0 ? notices.map((notice) => (
                  <div key={notice.id} className="p-5 border-l-4 border-emerald-500 bg-gray-50 rounded-r-xl hover:bg-emerald-50 transition-all">
                    <h4 className="font-bold text-gray-800 text-lg mb-1">{notice.title}</h4>
                    <p className="text-xs font-semibold text-emerald-600 mb-3 flex items-center"><Calendar className="h-3.5 w-3.5 mr-1.5"/> {notice.date}</p>
                    {notice.content && <p className="text-gray-600 text-sm leading-relaxed mb-3">{notice.content}</p>}
                    {notice.imageUrl && <img src={notice.imageUrl} alt="Notice" className="mt-2 rounded-lg max-h-40 w-auto object-cover border" />}
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {gallery.length > 0 ? gallery.map((img) => (
              <div key={img.id} className="relative group overflow-hidden rounded-2xl shadow-sm aspect-square bg-gray-100 border border-gray-200">
                ...
                {img.caption && <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-5 translate-y-[110%] group-hover:translate-y-0 transition-transform"><p className="text-white text-sm font-medium">{img.caption}</p></div>}
              </div>
      ...
      <footer className="bg-gray-900 text-gray-300 pt-16 pb-8 mt-auto border-t-4 border-emerald-600">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-12">
          ...
          <div>
            <h3 className="text-white font-bold text-lg mb-6 flex items-center"><span className="w-8 h-1 bg-emerald-500 mr-3 rounded-full"></span> লিংক</h3>
            <ul className="space-y-4">
              <li><a href="#about" className="text-gray-400 hover:text-emerald-400 flex items-center transition-colors"><ChevronRight className="mr-2" size={16}/> সম্পর্কে</a></li>
              <li><a href="#notices" className="text-gray-400 hover:text-emerald-400 flex items-center transition-colors"><ChevronRight className="mr-2" size={16}/> নোটিশ</a></li>
              <li><a href="#gallery" className="text-gray-400 hover:text-emerald-400 flex items-center transition-colors"><ChevronRight className="mr-2" size={16}/> গ্যালারি</a></li>
            </ul>
          </div>
          <div>
            <h3 className="text-white font-bold text-lg mb-6 flex items-center"><span className="w-8 h-1 bg-emerald-500 mr-3 rounded-full"></span> যোগাযোগ</h3>
            <ul className="space-y-5">
              <li className="flex items-start"><MapPin className="h-5 w-5 mr-3 text-emerald-500 mt-1"/> <span className="text-gray-400 leading-snug">{settings.address}</span></li>
              <li className="flex items-center"><Phone className="h-5 w-5 mr-3 text-emerald-500"/> <span className="text-gray-400">{settings.phone}</span></li>
              <li className="flex items-center"><Mail className="h-5 w-5 mr-3 text-emerald-500"/> <span className="text-gray-400 break-all">{settings.email}</span></li>
            </ul>
          </div>
        </div>
      ...
      {showAdmissionModal && <AdmissionModal onClose={() => setShowAdmissionModal(false)} firebaseUser={firebaseUser} db={db} appId={appId} />}
    </div>
  );
};

const AdmissionModal = ({ onClose, firebaseUser, db, appId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'admissions'), { ...formData, date: new Date().toLocaleDateString('bn-BD'), status: 'Pending' });
      setStatus('আপনার আবেদন সফলভাবে জমা হয়েছে! স্কুল থেকে যোগাযোগ করা হবে।');
      setTimeout(onClose, 3000);
    } catch (error) { setStatus('আবেদন জমা দিতে সমস্যা হয়েছে।'); }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative animate-in fade-in zoom-in duration-200">
        ...
        <div className="bg-emerald-600 px-8 py-6 text-white text-center"><h2 className="text-2xl font-bold">ভর্তির আবেদন ফর্ম</h2><p className="text-emerald-100 text-sm mt-1">২০২৬-২৭ শিক্ষাবর্ষ</p></div>
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          <div><label className="block text-sm font-bold text-gray-700 mb-2">শিক্ষার্থীর নাম</label><input required type="text" value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-bold text-gray-700 mb-2">অভিভাবকের মোবাইল</label><input required type="text" value={formData.phone} onChange={e=>setFormData({...formData, phone: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" /></div>
            <div><label className="block text-sm font-bold text-gray-700 mb-2">ভর্তির শ্রেণী</label><select value={formData.grade} onChange={e=>setFormData({...formData, grade: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white"><option>প্লে</option><option>নার্সারি</option><option>ক্লাস ১</option><option>ক্লাস ২</option><option>হিফজ সেকশন</option></select></div>
          </div>
          <div><label className="block text-sm font-bold text-gray-700 mb-2">বর্তমান ঠিকানা</label><textarea required value={formData.address} onChange={e=>setFormData({...formData, address: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" rows="2"></textarea></div>
          <button type="submit" disabled={status !== ''} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5 mt-4 disabled:opacity-50">আবেদন জমা দিন</button>
        </form>
      </div>
    </div>
  );
...
const LoginPage = ({ type, onLogin, onBack, onSwitchType }) => {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (!onLogin(username, password, type)) setError('ভুল ইউজারনেম বা পাসওয়ার্ড।'); };
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
          ...
          <div className="p-6 sm:p-10">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div><label className="block text-sm font-bold text-gray-700 mb-2">ইউজারনেম</label><input type="text" value={username} onChange={e => setUsername(e.target.value)} className={`w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-${themeColor}-500 focus:border-${themeColor}-500 outline-none transition-all shadow-sm`} placeholder="আপনার ইউজারনেম লিখুন" required /></div>
              <div><label className="block text-sm font-bold text-gray-700 mb-2">পাসওয়ার্ড</label><div className="relative"><input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} className={`w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-${themeColor}-500 focus:border-${themeColor}-500 outline-none transition-all shadow-sm pr-12`} placeholder="••••••••" required /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none"><Eye size={20}/></button></div></div>
              <button type="submit" className={`w-full bg-${themeColor}-600 hover:bg-${themeColor}-700 text-white font-bold text-lg py-3.5 px-4 rounded-xl shadow-lg hover:shadow-xl transition-all mt-2`}>প্রবেশ করুন</button>
            </form>
          </div>
...
const ChangePasswordModal = ({ user, onClose, onChangePassword, themeColor }) => {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (newPass !== confirmPass) return setStatus({ type: 'error', message: 'নতুন পাসওয়ার্ড মিলছে না!' }); if (onChangePassword(user.username, oldPass, newPass)) { setStatus({ type: 'success', message: 'সফলভাবে পরিবর্তন করা হয়েছে!' }); setTimeout(onClose, 2000); } else setStatus({ type: 'error', message: 'বর্তমান পাসওয়ার্ড ভুল!' }); };
  return (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div><label className="block text-sm font-bold mb-1">নতুন পাসওয়ার্ড নিশ্চিত করুন</label><input type="password" value={confirmPass} onChange={e=>setConfirmPass(e.target.value)} className="w-full border p-2 rounded-lg outline-none focus:ring-2" required /></div>
...
const DashboardLayout = ({ user, onLogout, onChangePassword, children }) => {
  ...
  const handleSidebarNav = (tab) => { setActiveTab(tab); setIsSidebarOpen(false); };
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      {isSidebarOpen && <div className="fixed inset-0 bg-black/60 z-40 lg:hidden" onClick={() => setIsSidebarOpen(false)} />}
      <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static inset-y-0 left-0 w-72 bg-${themeColor}-900 text-white flex flex-col shadow-2xl z-50 transition-transform duration-300`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto"><p className="px-4 text-xs font-semibold text-white/50 uppercase tracking-wider mb-2">মেন্যু</p>{isSMS ? (<><SidebarItem icon={<LayoutDashboard size={20}/>} label="ড্যাশবোর্ড ওভারভিউ" active={activeTab==='dashboard'} onClick={()=>handleSidebarNav('dashboard')} color={themeColor} /><SidebarItem icon={<Users size={20}/>} label="শিক্ষার্থী ভর্তি ও তালিকা" active={activeTab==='students'} onClick={()=>handleSidebarNav('students')} color={themeColor} /><SidebarItem icon={<Wallet size={20}/>} label="অ্যাকাউন্টস ও ফিন্যান্স" active={activeTab==='finance'} onClick={()=>handleSidebarNav('finance')} color={themeColor} /><SidebarItem icon={<FileCheck size={20}/>} label="অনলাইন আবেদন" active={activeTab==='admissions'} onClick={()=>handleSidebarNav('admissions')} color={themeColor} /></>) : (<><SidebarItem icon={<LayoutDashboard size={20}/>} label="ড্যাশবোর্ড ওভারভিউ" active={activeTab==='dashboard'} onClick={()=>handleSidebarNav('dashboard')} color={themeColor} /><SidebarItem icon={<Settings size={20}/>} label="ওয়েব সেটিংস ও লোগো" active={activeTab==='settings'} onClick={()=>handleSidebarNav('settings')} color={themeColor} /><SidebarItem icon={<FileText size={20}/>} label="নোটিশ বোর্ড (ছবিসহ)" active={activeTab==='notices'} onClick={()=>handleSidebarNav('notices')} color={themeColor} /><SidebarItem icon={<ImageIcon size={20}/>} label="গ্যালারি (ফাইল আপলোড)" active={activeTab==='gallery'} onClick={()=>handleSidebarNav('gallery')} color={themeColor} /></>)}</nav>
      </aside>
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full relative">
        <header className="bg-white h-20 shadow-sm flex items-center justify-between px-4 sm:px-8 z-10 border-b border-gray-100">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-gray-50/50">{React.cloneElement(children, { activeView: activeTab })}</div>
      </main>
...
const SidebarItem = ({ icon, label, active, color, onClick }) => (
...
const SMSDashboard = ({ firebaseUser, db, appId, activeView }) => {
  const [students, setStudents] = useState([]); const [admissions, setAdmissions] = useState([]); const [finances, setFinances] = useState([]);
  ...
  const showMsg = (msg) => { setNoticeMsg(msg); setTimeout(() => setNoticeMsg(''), 3000); };

  const handleAddStudent = async (e) => { e.preventDefault(); if (!firebaseUser || !name) return; await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'students'), { name, grade, date: new Date().toLocaleDateString('bn-BD') }); setName(''); showMsg('নতুন শিক্ষার্থী যুক্ত হয়েছে!'); };
  const handleAddFinance = async (e) => { e.preventDefault(); if (!firebaseUser || !finTitle || !finAmount) return; await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'finances'), { type: finType, category: finCategory, title: finTitle, amount: parseFloat(finAmount), date: new Date().toLocaleDateString('bn-BD') }); setFinTitle(''); setFinAmount(''); showMsg('হিসাব যুক্ত হয়েছে!'); };
  
  // NEW FEATURE: Accept Admission Function
  const handleAcceptAdmission = async (application) => {
    try {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'admissions', application.id), { status: 'গৃহীত' });
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'students'), { name: application.name, grade: application.grade, date: new Date().toLocaleDateString('bn-BD') });
      showMsg('আবেদনটি গৃহীত হয়েছে এবং শিক্ষার্থীর তালিকায় যুক্ত হয়েছে!');
    } catch (error) {
      showMsg('সমস্যা হয়েছে, আবার চেষ্টা করুন।');
    }
  };
  ...
  if (activeView === 'admissions') return (
    <div className="max-w-6xl mx-auto space-y-6">
      {noticeMsg && <div className="bg-green-100 text-green-700 p-4 rounded-xl font-bold">{noticeMsg}</div>}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"><div className="p-6 md:p-8 border-b bg-gray-50 flex justify-between items-center"><h3 className="font-bold text-xl text-gray-800 flex items-center"><FileCheck className="mr-3 text-emerald-600"/> অনলাইন ভর্তি আবেদনসমূহ</h3><span className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-sm font-bold">{admissions.length} টি আবেদন</span></div><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-white text-gray-500 text-sm uppercase"><tr className="border-b"><th className="p-4 px-6 font-semibold">নাম</th><th className="p-4 px-6 font-semibold">ফোন ও ঠিকানা</th><th className="p-4 px-6 font-semibold">শ্রেণী</th><th className="p-4 px-6 font-semibold">স্ট্যাটাস</th><th className="p-4 px-6 text-right">অ্যাকশন</th></tr></thead><tbody className="divide-y divide-gray-100">{admissions.length===0 ? <tr><td colSpan="5" className="text-center p-8 text-gray-400 font-medium">কোনো আবেদন নেই</td></tr> : admissions.map(a => (<tr key={a.id} className="hover:bg-gray-50"><td className="p-4 px-6 font-bold text-gray-800">{a.name}</td><td className="p-4 px-6"><p className="text-sm font-bold text-emerald-600">{a.phone}</p><p className="text-xs text-gray-500">{a.address}</p></td><td className="p-4 px-6 text-gray-600 font-bold">{a.grade}</td><td className="p-4 px-6"><span className={`px-3 py-1 rounded-full text-xs font-bold ${a.status === 'Pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>{a.status === 'Pending' ? 'অপেক্ষমাণ' : 'গৃহীত'}</span></td><td className="p-4 px-6 text-right space-x-2">{a.status === 'Pending' && <button onClick={() => handleAcceptAdmission(a)} className="text-green-500 hover:text-white hover:bg-green-500 p-2 rounded-lg transition-colors inline-flex" title="ভর্তি গ্রহণ করুন"><CheckCircle size={18}/></button>} <button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'admissions', a.id))} className="text-red-400 hover:text-white hover:bg-red-500 p-2 rounded-lg transition-colors inline-flex" title="মুছে ফেলুন"><Trash2 size={18}/></button></td></tr>))}</tbody></table></div></div>
    </div>
  );

  if (activeView === 'finance') {
    const totalIncome = finances.filter(f=>f.type==='Income').reduce((sum, curr)=>sum+curr.amount, 0); const totalExpense = finances.filter(f=>f.type==='Expense').reduce((sum, curr)=>sum+curr.amount, 0); const netBalance = totalIncome - totalExpense;
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        {noticeMsg && <div className="bg-green-100 text-green-700 p-4 rounded-xl font-bold">{noticeMsg}</div>}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100"><p className="text-gray-500 font-bold text-sm mb-1 flex items-center"><ArrowDownRight className="text-green-500 mr-1" size={16}/> মোট আয় (ফিস/অন্যান্য)</p><h3 className="text-3xl font-extrabold text-green-600">৳{totalIncome}</h3></div><div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100"><p className="text-gray-500 font-bold text-sm mb-1 flex items-center"><ArrowUpRight className="text-red-500 mr-1" size={16}/> মোট ব্যয় (বেতন/অন্যান্য)</p><h3 className="text-3xl font-extrabold text-red-600">৳{totalExpense}</h3></div><div className="bg-emerald-600 p-6 rounded-2xl shadow-sm border border-emerald-500 text-white"><p className="text-emerald-100 font-bold text-sm mb-1 flex items-center"><Wallet className="mr-1" size={16}/> বর্তমান ব্যালেন্স</p><h3 className="text-3xl font-extrabold">৳{netBalance}</h3></div></div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8"><h2 className="text-xl font-bold mb-6 text-gray-800 border-b pb-4">নতুন হিসাব যুক্ত করুন</h2><form onSubmit={handleAddFinance} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4"><div className="md:col-span-1"><label className="block text-xs font-bold text-gray-500 mb-1">হিসাবের ধরন</label><select value={finType} onChange={(e)=>{setFinType(e.target.value); setFinCategory(e.target.value==='Income'?'ছাত্রীদের বেতন (Fees)':'শিক্ষক ও স্টাফদের বেতন (Payroll)');}} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 outline-none font-bold bg-white"><option value="Income">আয় (Income)</option><option value="Expense">ব্যয় (Expense)</option></select></div><div className="md:col-span-1"><label className="block text-xs font-bold text-gray-500 mb-1">ক্যাটাগরি</label><select value={finCategory} onChange={e=>setFinCategory(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 outline-none font-bold bg-white">{finType === 'Income' ? (<><option>ছাত্রীদের বেতন (Fees)</option><option>ভর্তি ফি</option><option>অন্যান্য আয়</option></>) : (<><option>শিক্ষক ও স্টাফদের বেতন (Payroll)</option><option>ইউটিলিটি বিল</option><option>অফিস খরচ</option></>)}</select></div><div className="md:col-span-1"><label className="block text-xs font-bold text-gray-500 mb-1">পরিমাণ (৳)</label><input type="number" placeholder="00.00" value={finAmount} onChange={e=>setFinAmount(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 outline-none font-bold" required/></div><div className="md:col-span-1"><label className="block text-xs font-bold text-gray-500 mb-1">বিবরণ / নাম</label><input type="text" placeholder="যেমন: রহিম এর বেতন" value={finTitle} onChange={e=>setFinTitle(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-2 outline-none font-medium" required/></div><div className="md:col-span-1 flex items-end"><button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white p-3 rounded-xl font-bold shadow-md transition-all h-[46px]">যোগ করুন</button></div></form></div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"><div className="p-6 border-b bg-gray-50"><h3 className="font-bold text-xl text-gray-800">সাম্প্রতিক লেনদেন</h3></div><table className="w-full text-left"><thead className="bg-white text-gray-500 text-sm uppercase"><tr className="border-b"><th className="p-4 px-6 font-semibold">বিবরণ</th><th className="p-4 px-6 font-semibold">ক্যাটাগরি</th><th className="p-4 px-6 font-semibold">পরিমাণ</th><th className="p-4 px-6 text-right">অ্যাকশন</th></tr></thead><tbody className="divide-y divide-gray-100">{finances.length===0 ? <tr><td colSpan="4" className="text-center p-8 text-gray-400">কোনো ডাটা নেই</td></tr> : finances.map(f => (<tr key={f.id} className="hover:bg-gray-50"><td className="p-4 px-6 font-bold text-gray-800">{f.title}<br/><span className="text-xs text-gray-400 font-normal">{f.date}</span></td><td className="p-4 px-6 text-sm text-gray-600 font-medium">{f.category}</td><td className={`p-4 px-6 font-extrabold ${f.type==='Income'?'text-green-600':'text-red-600'}`}>{f.type==='Income'?'+':'-'}৳{f.amount}</td><td className="p-4 px-6 text-right"><button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'finances', f.id))} className="text-red-400 hover:text-white hover:bg-red-500 p-2 rounded-lg transition-colors"><Trash2 size={16}/></button></td></tr>))}</tbody></table></div>
      </div>
    );
  }

  return (
    <div className="max-w-6xl mx-auto space-y-6"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"><div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100 flex items-center space-x-6"><div className="bg-blue-500 text-white p-4 rounded-2xl shadow-lg shadow-blue-500/30"><Users size={32}/></div><div><p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">মোট শিক্ষার্থী</p><h3 className="text-4xl font-extrabold text-gray-800">{students.length}</h3></div></div><div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100 flex items-center space-x-6"><div className="bg-emerald-500 text-white p-4 rounded-2xl shadow-lg shadow-emerald-500/30"><FileCheck size={32}/></div><div><p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">নতুন আবেদন</p><h3 className="text-4xl font-extrabold text-gray-800">{admissions.length}</h3></div></div></div><div className="bg-gradient-to-br from-emerald-50 to-white rounded-2xl shadow-sm border border-emerald-100 p-10 relative overflow-hidden"><h3 className="text-2xl font-bold mb-3 text-emerald-900">স্বাগতম, স্কুল অ্যাডমিন!</h3><p className="text-gray-600 max-w-lg text-lg">বাম পাশের মেন্যু থেকে শিক্ষার্থী পরিচালনা, অনলাইন আবেদন গ্রহণ এবং সম্পূর্ণ ফিন্যান্স (আয়/ব্যয়) ম্যানেজ করতে পারবেন।</p></div></div>
  );
};

const CMSDashboard = ({ firebaseUser, db, appId, activeView }) => {
  const [settings, setSettings] = useState(defaultSiteSettings); const [notices, setNotices] = useState([]); const [gallery, setGallery] = useState([]);
  ...
  useEffect(() => {
    if (!firebaseUser || !db) return;
    try {
      onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'main_config'), (docSnap) => { if (docSnap.exists()) setSettings({ ...defaultSiteSettings, ...docSnap.data() }); });
  ...
  const handleLogoUpload = async (e) => { try { setSettings({ ...settings, logoUrl: await processImage(e.target.files[0]) }); } catch (err) { alert('ছবি আপলোডে সমস্যা হয়েছে।'); } };
  const handleNoticeImgUpload = async (e) => { try { setNoticeImgBase64(await processImage(e.target.files[0])); } catch (err) {} };
  const handleGalleryImgUpload = async (e) => { try { setGalImgBase64(await processImage(e.target.files[0])); } catch (err) {} };

  const handleSaveSettings = async (e) => { e.preventDefault(); setSaveStatus('সংরক্ষণ করা হচ্ছে...'); try { await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'main_config'), settings, { merge: true }); setSaveStatus('সফলভাবে সংরক্ষিত হয়েছে!'); setTimeout(() => setSaveStatus(''), 3000); } catch (error) { setSaveStatus('ত্রুটি হয়েছে!'); } };
  const handleAddNotice = async (e) => { e.preventDefault(); if (!noticeTitle) return; await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'notices'), { title: noticeTitle, content: noticeContent, imageUrl: noticeImgBase64, date: new Date().toLocaleDateString('bn-BD') }); setNoticeTitle(''); setNoticeContent(''); setNoticeImgBase64(''); };
  const handleAddGallery = async (e) => { e.preventDefault(); if (!galImgBase64) return alert("দয়া করে একটি ছবি সিলেক্ট করুন।"); await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'gallery'), { url: galImgBase64, caption: imgCaption }); setGalImgBase64(''); setImgCaption(''); };

  if (activeView === 'settings') return (
    <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10">
      <h2 className="text-2xl font-bold mb-8 text-gray-800 border-b pb-4 flex items-center"><Settings className="mr-3 text-blue-600"/> ওয়েবসাইট সেটিংস ও লোগো</h2>
      {saveStatus && <div className={`mb-6 p-4 rounded-xl font-bold flex items-center ${saveStatus.includes('সফল') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}><CheckCircle className="mr-2"/> {saveStatus}</div>}
  ...
  if (activeView === 'gallery') return (
    <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10">
      <h2 className="text-2xl font-bold mb-8 text-gray-800 border-b pb-4 flex items-center"><ImageIcon className="mr-3 text-blue-600"/> গ্যালারি ম্যানেজমেন্ট (ফাইল আপলোড)</h2>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6"><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center"><Globe className="h-16 w-16 text-blue-500 mb-4 bg-blue-50 p-3 rounded-2xl"/><h3 className="font-bold text-xl text-gray-800">ওয়েব সেটিংস</h3></div><div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center"><FileText className="h-16 w-16 text-emerald-500 mb-4 bg-emerald-50 p-3 rounded-2xl"/><h3 className="font-bold text-xl text-gray-800">মোট নোটিশ: {notices.length}</h3></div></div></div>
  );
};